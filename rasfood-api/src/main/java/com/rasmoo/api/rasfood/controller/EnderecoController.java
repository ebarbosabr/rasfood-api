...
public class EnderecoController {
    ...
    @GetMapping("/cep/{cep}")
    public ResponseEntity<List<Endereco>> consultarPorCep(@PathVariable("cep") String cep) {
        return ResponseEntity.status(HttpStatus.OK).body(enderecoRepository.findByCep(cep));
    }

    @GetMapping("/{id}")
    public ResponseEntity<Endereco> consultarPorId(@PathVariable("id") Integer id) {
        return enderecoRepository.findById(id).map(value -> ResponseEntity.status(HttpStatus.OK).body(value)).orElseGet(() -> ResponseEntity.status(HttpStatus.NOT_FOUND).body(null));
    }

    @PatchMapping("/{id}")
    public ResponseEntity<Endereco> atualizar(@PathVariable("id") Integer id,
                                              @RequestBody Endereco endereco) throws JsonMappingException {
